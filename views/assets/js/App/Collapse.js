class Collapse {
    constructor() {
        const controls = Array.prototype.slice.call(document.querySelectorAll('[aria-expanded][aria-controls]'))
        this.collapsibleElements = controls.map((control) => [control, document.getElementById(control.getAttribute('aria-controls'))])
        this.handleClick = this.handleClick.bind(this)
        this.init()
    }

    handleClick({ target: control }) {
        const container = document.getElementById(control.getAttribute('aria-controls'))
        if (!container) {
            return false
        }
    ...
    init() {
        ...
        this.collapsibleElements.forEach(function ([control, container]) {
            ...
            container.addEventListener('transitionend', instance.handleTransitionEnd)
        ...
        window.addEventListener('resize', function () {
            ...
            timer = window.setTimeout(function () {
                instance.collapsibleElements.forEach(function ([control, container]) {
                    instance.resetNaturalHeight(control, container)
                })
            }, 50)
        })
    ...
    resetNaturalHeight(control, container) {
        ...
        const initiallyVisible = control.getAttribute('aria-expanded') === 'true'
